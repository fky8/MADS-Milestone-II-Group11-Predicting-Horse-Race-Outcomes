...
# Load race metadata from 2020 file
metadata_path = os.path.join(script_dir, "RacePlaceData_2020.csv")
race_meta_df = pd.read_csv(metadata_path)
race_meta_df = race_meta_df[["Date", "Course", "RaceNumber"]]
race_meta_df = race_meta_df.drop_duplicates()
...
for _, row in race_meta_df.iterrows():
    ds = row["Date"]
    course = row["Course"]
    race_no = int(row["RaceNumber"])
    new_date = f"Date: {ds}"
    if new_date != locals().get("last_printed_date", ""):
        print(new_date)
        last_printed_date = new_date
    ...
    try:
        ...
        print(f"    – Race#{race_no} missing → skipping")
        continue

    html = tbl.get_attribute("outerHTML")
    tbls = pd.read_html(StringIO(html))
    if not tbls:
        print(f"    – Race#{race_no} empty → skipping")
        continue
...
# Combine all collected data and save
if df_all:
    df_combined = pd.concat(df_all, ignore_index=True)
    output_path = os.path.join(script_dir, "RacePlaceData_from_2020_metadata.csv")
    df_combined.to_csv(output_path, index=False)
    print(f"\nScraping complete: saved {len(df_combined)} rows → {output_path}")
else:
