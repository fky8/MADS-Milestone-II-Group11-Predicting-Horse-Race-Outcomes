...
jockey_race_data['Top3'] = (
    jockey_race_data['Placing'].str.extract(r'^(\d+)/')[0]
    .astype(int)
    .le(3)
    .astype(int)
)

jockey_race_data = jockey_race_data.drop(columns=[
    'Placing', 'Unnamed: 13', 'race_wins', 'first_place_win',
    'second_place_win', 'third_place_win', 'Race Index', 'Race Sub Index', 'Rtg.'
], errors='ignore')

weather_data = weather_data.drop(columns=[
    'MaximumRelativeHumidity', 'MinimumRelativeHumidity', 'GrassMinimumTemperature',
    'Rainfall(mm)', 'MeanUVIndex', 'MaxUVIndex'
], errors='ignore')

X = pd.merge(jockey_race_data, weather_data, how='left', left_on='race_date', right_on='Date')
X = X.drop(columns=['Date'], errors='ignore')

y = X['Top3']
X = X.drop(columns=['Top3'])

leak_cols = [col for col in X.columns if 'Placing' in col or 'Finish Time' in col]
X = X.drop(columns=leak_cols, errors='ignore')

scores = []
all_preds = []
all_truths = []
coef_accum = defaultdict(float)
feature_names = None
...
importance_df = pd.DataFrame({
    'feature': list(coef_accum.keys()),
    'total_importance': list(coef_accum.values())
})
...
plt.title('Daily F1 Score Over Time (Baseline Logistic Regression)')
