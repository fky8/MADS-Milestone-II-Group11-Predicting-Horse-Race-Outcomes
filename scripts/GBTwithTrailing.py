...
horse_feats = pd.read_csv("data\horse_trailing_stats_Score range.csv", parse_dates=["Date"])
jockey_feats = pd.read_csv("data\Jockey_trailing_stats_Score range.csv", parse_dates=["Date"])
...
jockey_race_data = jockey_race_data.drop(columns=['Placing', 'Unnamed: 13', 'race_wins', 'first_place_win','second_place_win', 'third_place_win', 'Race Index', 'Race Sub Index','Rtg.'], errors='ignore')
...
horse_feats = horse_feats.rename(columns={"Date": "race_date"})
jockey_feats = jockey_feats.rename(columns={"Date": "race_date"})
X = pd.merge(X, horse_feats, how="left", on=["race_date", "Horse"])
X = pd.merge(X, jockey_feats, how="left", on=["race_date", "Jockey"])
