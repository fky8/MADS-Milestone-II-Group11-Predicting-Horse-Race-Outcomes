...
# best opponent embeddings
df_jockey_grouped = df_jockey.groupby(['Date', 'RaceNumber'])
numeric_cols = df_jockey.select_dtypes(include=[np.number]).columns

largest_values = {}
for col in numeric_cols:
    largest_values[col] = df_jockey_grouped.apply(lambda x: x.nlargest(4, col)).reset_index(drop=True)

for col, values in largest_values.items():
    df_jockey_grouped[col + '_rank'] = values
...
# best opponent embeddings
df_jockey_grouped = df_jockey.groupby(['Date', 'RaceNumber']).nlargest(4).reset_index()
numeric_cols = df_jockey_grouped.select_dtypes(include=[np.number]).columns

largest_values = {}
for col in numeric_cols:
    largest_values[col] = df_jockey_grouped.groupby(['Date', 'RaceNumber'])[col].nlargest(4, cols=col).reset_index(drop=True)

for col, values in largest_values.items():
    df_jockey_grouped[col + '_rank'] = values

first_max = df_jockey_grouped.groupby(['Date', 'RaceNumber']).nth(0).reset_index()
second_max = df_jockey_grouped.groupby(['Date', 'RaceNumber']).nth(1).reset_index()
third_max = df_jockey_grouped.groupby(['Date', 'RaceNumber']).nth(2).reset_index()
fourth_max = df_jockey_grouped.groupby(['Date', 'RaceNumber']).nth(3).reset_index()

for col in j_emb_cols:
    first_max.rename(columns={col: col + '_1st'}, inplace=True)
    second_max.rename(columns={col: col + '_2nd'}, inplace=True)
    third_max.rename(columns={col: col + '_3rd'}, inplace=True)
    fourth_max.rename(columns={col: col + '_4th'}, inplace=True)

df_horse_grouped = df_horse.groupby(['Date', 'RaceNumber']).max().reset_index()
first_max_horse = df_horse_grouped.groupby(['Date', 'RaceNumber']).nth(0).reset_index()
second_max_horse = df_horse_grouped.groupby(['Date', 'RaceNumber']).nth(1).reset_index()
third_max_horse = df_horse_grouped.groupby(['Date', 'RaceNumber']).nth(2).reset_index() 
fourth_max_horse = df_horse_grouped.groupby(['Date', 'RaceNumber']).nth(3).reset_index()

for col in h_emb_cols:
    first_max_horse.rename(columns={col: col + '_1st'}, inplace=True)
    second_max_horse.rename(columns={col: col + '_2nd'}, inplace=True)  
    third_max_horse.rename(columns={col: col + '_3rd'}, inplace=True)
    fourth_max_horse.rename(columns={col: col + '_4th'}, inplace=True)

df_data = df_data.merge(first_max, on=['Date', 'RaceNumber'], how='left')
df_data = df_data.merge(second_max, on=['Date', 'RaceNumber'], how='left')
df_data = df_data.merge(third_max, on=['Date', 'RaceNumber'], how='left')
df_data = df_data.merge(fourth_max, on=['Date', 'RaceNumber'], how='left')
df_data = df_data.merge(first_max_horse, on=['Date', 'RaceNumber'], how='left')
df_data = df_data.merge(second_max_horse, on=['Date', 'RaceNumber'], how='left')
df_data = df_data.merge(third_max_horse, on=['Date', 'RaceNumber'], how='left')
df_data = df_data.merge(fourth_max_horse, on=['Date', 'RaceNumber'], how='left')

# delta between opponent mean embedding and the horse/jockey embedding
for col in j_emb_cols:
    df_data[col + '_delta_1st'] = abs(df_data[col] - df_data[col + '_1st'])
    df_data[col + '_delta_2nd'] = abs(df_data[col] - df_data[col + '_2nd'])
    df_data[col + '_delta_3rd'] = abs(df_data[col] - df_data[col + '_3rd'])
    df_data[col + '_delta_4th'] = abs(df_data[col] - df_data[col + '_4th'])
...
for effective_date in effective_date_list:
    ...
    umap_cols = [col for col in cols if col[0:6] in ['H_UMAP_', 'J_UMAP']]
    # embedding_cols = [col for col in cols if col[0:6] in ['H_Emb_']]

    emb_delta_1st_opp_cols = [col for col in cols if '_delta_1st' in col and 'Emb_' in col]
    emb_delta_2nd_opp_cols = [col for col in cols if '_delta_2nd' in col and 'Emb_' in col]
    emb_delta_3rd_opp_cols = [col for col in cols if '_delta_3rd' in col and 'Emb_' in col]
    emb_delta_4th_opp_cols = [col for col in cols if '_delta_4th' in col and 'Emb_' in col]
    ...
    emb_1st_opp_cols = [col for col in cols if '_1st' in col and 'Emb_' in col]
    emb_2nd_opp_cols = [col for col in cols if '_2nd' in col and 'Emb_' in col]
    emb_3rd_opp_cols = [col for col in cols if '_3rd' in col and 'Emb_' in col]
    emb_4th_opp_cols = [col for col in cols if '_4th' in col and 'Emb_' in col]
    emb_min_opp_cols = [col for col in cols if '_min' in col and 'Emb_' in col]


    umap_delta_1st_opp_cols = [col for col in cols if '_delta_1st' in col and 'UMAP_' in col]
    umap_delta_2nd_opp_cols = [col for col in cols if '_delta_2nd' in col and 'UMAP_' in col]
    umap_delta_3rd_opp_cols = [col for col in cols if '_delta_3rd' in col and 'UMAP_' in col]
    umap_delta_4th_opp_cols = [col for col in cols if '_delta_4th' in col and 'UMAP_' in col]
    ...
    umap_1st_opp_cols = [col for col in cols if '_1st' in col and 'UMAP_' in col]
    umap_2nd_opp_cols = [col for col in cols if '_2nd' in col and 'UMAP_' in col]
    umap_3rd_opp_cols = [col for col in cols if '_3rd' in col and 'UMAP_' in col]
    umap_4th_opp_cols = [col for col in cols if '_4th' in col and 'UMAP_' in col]
    ...
    for i in range(0, 1):
        ...
        features = set(keep_cols
                    ...
                    + embedding_cols
                    + emb_delta_1st_opp_cols
                    + emb_delta_2nd_opp_cols
                    + emb_delta_3rd_opp_cols
                    + emb_delta_4th_opp_cols
                    + emb_1st_opp_cols
                    + emb_2nd_opp_cols
                    + emb_3rd_opp_cols
                    + emb_4th_opp_cols
                    + emb_delta_min_opp_cols
                    + emb_delta_mean_opp_cols
                    + emb_mean_opp_cols
                    + emb_min_opp_cols
                    + emb_1st_opp_cols 
                    + emb_2nd_opp_cols 
                    ...
                    + umap_cols
                    + umap_delta_1st_opp_cols
                    + umap_delta_2nd_opp_cols
                    + umap_delta_3rd_opp_cols
                    + umap_delta_4th_opp_cols
