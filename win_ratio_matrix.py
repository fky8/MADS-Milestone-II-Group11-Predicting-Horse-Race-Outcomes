...
number_of_trailing_days = 365 # last two years of data
number_of_trailing_days = 28 # last two years of data
trailing_days = 360
...
for i in range(0, number_of_trailing_days, 7):
    ...
    try:
        ...
        # df_embedding = pd.concat([pd.DataFrame(v) for k, v in embedding.items()], axis=1, keys=list(embedding.keys()))
        df_embedding = pd.DataFrame(list(embedding.items()), columns=['Target Feature', 'Embedding'])
        df_embedding['Date'] = end_date.strftime('%Y-%m-%d')
...
df_embeddings_all = pd.concat(df_embeddings, axis=1)

script_dir = os.path.dirname(os.path.abspath(__file__)) 
file_path = os.path.join(script_dir, f"data/" + target_feature + "_" + comparison_feature +".csv")
